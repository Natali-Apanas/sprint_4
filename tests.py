...
class TestBooksCollector:
    # 1
    def test_add_new_book_with_empty_title_does_not_add(self):
    ...
    @pytest.mark.parametrize(
        'name, genre',
        [
            ['Кладбище домашних животных', 'Фентези'],
            ['Лето в Простоквашино', 'Аниме'],
            ['Снежная королева', 'Драма']
        ]
    )
    def test_get_book_genre_unsupported_genre_not_returned(self, name, genre):
        collector = BooksCollector()
        collector.add_new_book(name)
        collector.set_book_genre(name, genre)
    ...
    def test_get_books_with_specific_genre_returns_right_books_list(self, books, genre):
        collector = BooksCollector()
        for book in books:
    ...
    def test_get_books_genre_returns_correct_dict(self, books_dict):
        collector = BooksCollector()
    ...
    # 7
    def test_get_books_for_children_returns_correct_books_for_children(self):
        collector = BooksCollector()
    ...
    # 8
    def test_add_book_in_favorites_not_add_unkown_book_in_favorites(self):
        collector = BooksCollector()
        collector.add_book_in_favorites('Кладбище домашних животных')
        assert len(collector.get_list_of_favorites_books()) == 0

    # 9
    def test_delete_book_from_favorites_delete_only_one_book_from_favorites(self):
        collector = BooksCollector()
